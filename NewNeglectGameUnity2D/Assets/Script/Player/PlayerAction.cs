...
public class PlayerAction : Action
{
    ...
    //{
    //    curState = STATE.WALK;
    //    moveTargetEnemy = null; // 타겟 몬스터를 정함.
    //    float distance = float.MaxValue;
    //    var getEnemyList = EnemyManager.GetEnemyList();
    //    foreach(var enemyObject in getEnemyList)
    //    {
    ...
    //            {
    //                distance = newDistance;
    //                moveTargetEnemy = enemyObject.Value;
    //            }
    //        }
    //    }
    //}
   ...
   // {
   //     Vector3 playerPos = this.GetComponent<Transform>().position;
   //     Vector3 TargetPos = moveTargetEnemy.GetComponent<Transform>().position;
   ...
   //         // 이동중
   //         curState = STATE.WALK;
   //         Vector3 newPosition = Vector3.MoveTowards(rb2D.position, TargetPos, playerSpeed * Time.deltaTime);   // MoveTowards 리지드바디 2D의 움직임을 계산.
   ...
   // }
    ...
    //public IEnumerator AttackToMonster()
    //{
    //    curState = STATE.ATTACK;
    //    if (animator.GetBool("isAttack"))
    //    {
    //        //Debug.Log("공격 중");
    //        StartCoroutine(moveTargetEnemy.GetComponent<Enemy>().DamageCharacter(playerAtk, animator.speed));
    //        yield return new WaitForSeconds(animator.speed);
    //        AttackOff();
    //        //Debug.Log("공격 끝");
    //    }
